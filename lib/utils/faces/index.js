...
const { map, round, difference } = require('lodash');
const bodyPix = require('@tensorflow-models/body-pix');
const tjfs = require('@tensorflow/tfjs');
...
async function loadImageIntoCanvas(path) {
...
const getBodyMap = async (imagePath) => {
  await init();
  const net = await bodyPix.load();
  const image = await loadImageIntoCanvas(imagePath);
  const c = canvas.createCanvas(image.width, image.height);
  const ctx = c.getContext('2d');
  ctx.drawImage(image, 0, 0);
  const input = tjfs.browser.fromPixels(c);
  const segments = await net.segmentMultiPerson(input);
