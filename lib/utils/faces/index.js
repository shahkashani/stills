...
const { map, round, difference, sortBy } = require('lodash');
...
const getBodyMap = async (imagePath, segmentationThreshold = 0.7) => {
  const net = await bodyPix.load();
  const file = await readFileSync(imagePath);
  const imageData = await tjfs.node.decodeImage(file, 3);
  const segments = await net.segmentMultiPerson(imageData, {
