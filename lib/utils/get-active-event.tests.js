...
describe('getActiveEvent', () => {
  ...
  it('accepts configurable end times', () => {
    const events = {
      'February 24 10:30 EST -> February 24 11:30 EST': 'tp'
    };
    expect(
      getActiveEvent(events, new Date('Feb 24 2024 10:30:00 EST'))
    ).toEqual({ event: 'tp', progress: 0 });
    ...
    expect(
      getActiveEvent(events, new Date('Feb 24 2024 11:30:00 EST'))
    ).toEqual({ event: 'tp', progress: 1 });

    expect(
      getActiveEvent(events, new Date('Feb 24 2024 11:31:00 EST'))
    ).toEqual(null);
