...
const getAngles = (
  ...
  const prev = Math.atan2(prevpoint.y - currpoint.y, prevpoint.x - currpoint.x),
    ...
    main = Math.acos(
      (prevlen * prevlen + nextlen * nextlen - mainlen * mainlen) /
        (2 * prevlen * nextlen)
    ),
...
const getMinHit = (arr) =>
  arr.reduce(
    (min, p) => (p.locked ? min : !min ? p : p.arc.hit < min.arc.hit ? p : min),
    null
  );
...
const getSegments = (points, type, opt) =>
  points.reduce((segmented, point) => {
    const {
        x,
        y,
        angle: { prev, next },
        arc: { x: rx, y: ry, radius }
      } = point,
      ...
      amount =
        type === 'LENGTH'
          ? Math.round((dir * angle * radius) / opt)
          : (opt || 1) - 1,
