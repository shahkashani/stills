...
class ContentGif {
  ...
  generate(input, output) {
    ...
    shell(
      `ffmpeg -ss ${sec} -t ${duration} -i "${input}" -v warning -filter_complex "[0:v] fps=${fps},scale=w=${width}:h=-1,split [a][b];[a] palettegen=reserve_transparent=off:stats_mode=single [p];[b][p] paletteuse=new=1" "${outputFile}"`
    );
