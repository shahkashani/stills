...
const getTimestamp = (filename, seconds, isPreferBeginning = false) => {
  if (Number.isFinite(seconds)) {
    return seconds;
  }
  const beginning = 3 / 4;
  const length = getVideoLength(filename);
  const usableLength = isPreferBeginning ? length * beginning : length;
  return getRandomTimestamp(usableLength);
};
...
const getTimestamps = (
  ...
  const results = [];
  const start = getTimestamp(filename, seconds, isPreferBeginning);
