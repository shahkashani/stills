...
class DestinationTumblr {
  ...
  getAlt(index, captions, analysis) {
    const a =
  ...
  async createPhotoPostNpf(images, tags, rawText, captions, analysis) {
    ...
    const imageContent = images.map((image, index) => {
      return {
        type: 'image',
        alt_text: this.getAlt(index, captions, analysis),
        media: [
          {
  ...
  async publish(images, { text = null, tags = [] } = {}, results) {
