...
class DestinationTumblr {
  ...
  getMatchRegexps(words, isLoose = false) {
    const escapeRegExp = (string) =>
      string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    return words.map((word) => {
      const cleanedWord = escapeRegExp(word);
      return isLoose
        ? new RegExp(`${cleanedWord}`, 'i')
        : new RegExp(`\\b${cleanedWord}\\b`, 'i');
    });
  }
  ...
  highlightWords(text, words) {
    ...
    const index1 = text.indexOf('Caption:');
    const index2 = text.indexOf('Captions:');
    let index = 0;
    if (index1 !== -1) {
      index = index1;
    } else if (index2 !== -1) {
    ...
    const getFormats = (regexps) =>
      compact(
        regexps.map((regexp, i) => {
          const m = regexp.exec(relevantText);
          if (!m) {
            return null;
          }
          return {
            start: index + m.index,
            end: index + m.index + words[i].length,
            type: 'color',
            hex: WORD_HEX
          };
        })
      );
    const proper = getFormats(this.getMatchRegexps(words));
    if (proper.length > 0) {
      return proper;
    }
    return getFormats(this.getMatchRegexps(words, true));
  }
