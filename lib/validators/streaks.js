...
class ValidatorStreaks {
  ...
  async repairStreaks(image, streaks) {
    ...
    console.log('👨‍🎤 Ready to close some gaps!');

    for (const gap of streaks) {
      for (const index of gap.indexes) {
        console.log(`👨‍🎤 Deleting frame index`, index);
        image.deleteFrame(index);
      }
    }
    image.collapse();
    return true;
  ...
  getInvalidStreaks = (streaks, frames) => {
    const results = [];
    const minSceneLength = Math.round(frames.length * this.minSceneLength);
    for (const streak of streaks) {
      const sceneLength = this.getSceneLength(streak);
      if (sceneLength < minSceneLength) {
        console.log(
          `👨‍🎤 This entire scene is too short (${sceneLength}, should be at least ${minSceneLength}).`
        );
        results.push(...streak);
      } else {
  ...
  async validate(image) {
    ...
    const invalidStreaks = this.getInvalidStreaks(streaks, image.frames);
