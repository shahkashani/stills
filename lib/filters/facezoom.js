...
class FilterFaceZoom {
  ...
  getInstructions(imageWidth, imageHeight, face, frameCount) {
    const { x, y, width, height } = face;
    const deltaX = x / frameCount;
    const deltaY = y / frameCount;
    const deltaH = (imageHeight - height) / frameCount;
    const deltaW = (imageWidth - width) / frameCount;
    const imageRatio = imageWidth / imageHeight;

    const startAt = Math.floor(frameCount * this.startPosition);

    return range(startAt, frameCount).map(index => {
  ...
  async apply(file) {
    ...
    const tempStillFrame = this.getStill(file, frameCount - 1);
