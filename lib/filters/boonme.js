...
class FilterBoonme {
  ...
  fillFaces(faces) {
    let i = 0;
    const count = Object.values(faces).length;
    const results = { ...faces };

    for (i = 1; i < count - 1; i += 1) {
      const shouldFillFace =
        results[i].length !== results[i - 1].length &&
        results[i + 1].length > 0 &&
        results[i - 1].length > 0;
      if (shouldFillFace) {
        console.log('👍 Filled a face gap!');
      }
      results[i] = shouldFillFace ? results[i - 1] : results[i];
    }
    return results;
  }
  ...
  async apply(file) {
    ...
    await processAsStills(
      ...
      async (png, progress, index) => {
        facesPerFrames[index] = await getFaces(png, this.avoidDescriptors);
        if (progress === 1) {
          facesPerFrames = this.fillFaces(facesPerFrames);
        }
      }
    );
  }
