...
class FilterSymbol {
  constructor({
    ...
    symbolRatioSize = 0.5,
    symbolSize = '174x',
    symbolGravity = 'northeast',
    symbolGeometry = '+31+13',
    isTrace = false,
    fillColor = 'white',
    symbolEffects = {
      grayscale: false,
  ...
  async applySymbol(symbol, frame, image) {
    ...
    exec(
      frame,
      `-coalesce NULL: \\( "${symbol}" -resize ${size} ${grayscale} ${negate} ${useBorder} -matte -channel A +level 0,${this.symbolOpacity}% \\) -gravity ${this.symbolGravity} -geometry ${this.symbolGeometry} -layers composite`
    );
  }
