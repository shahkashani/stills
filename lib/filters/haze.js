...
class FilterHaze {
  ...
  glowingEyes(png, faces) {
    const eyes = faces.reduce((memo, face) => {
      ...
      const { width: rightWidth } = getBoundingBox(face.landmarks.getLeftEye());
    ...
    this.bodies.forEach(({ blur, fill, opacity, scale = 1 }) => {
      ...
      execCmd(
        `convert "${png}" \\( +clone -fill "${fill}" -colorize 100 \\( ${mask} \\) -alpha off -compose CopyOpacity -composite ${o} ${b} \\) -compose over -dither None -composite "${png}"`
      );
    });
  }
  ...
  async apply(file, getImageInfo) {
    ...
    await processAsStills(file, async (png) => {
      ...
      execCmd(`convert "${png}" ${this.base} -dither None "${png}"`);
      if (faces.length > 0) {
