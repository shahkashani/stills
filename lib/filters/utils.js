...
const getDebugRectCmd = (box, color = 'blue') => {
  const { x, y, width, height } = box;
  const x2 = x + width;
  const y2 = y + height;
  return `-fill none -stroke ${color} -draw "rectangle ${x},${y} ${x2},${y2}"`;
};

const getDebugPolygonCmd = (points, color = 'blue') => {
  const coords = points.map(({ x, y }) => `${x},${y}`).join(' ');
  return `-fill none -stroke ${color} -draw "polygon ${coords}"`;
};
