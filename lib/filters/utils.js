...
const getLastScene = async file => {
  ...
  const numLateFrames = Math.ceil(numFrames - numFrames / 4);
  const lateChangesRemoved = changes.filter(
    change => change.frame < numLateFrames
  );
  return {
...
const getProgressiveCmd = (callback, frameEnd, frameStart = 0) => {
  const numFrames = frameEnd - frameStart;
  const instructions = range(frameStart, frameEnd + 1).map(
    (frame, i) => `\\( -clone ${frame} ${callback(i / numFrames)} \\)`
  );
  return `${instructions.join(
    ' '
  )} -delete ${frameStart}-${frameEnd} -coalesce`;
};
