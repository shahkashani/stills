...
const { map, range, sortBy, get } = require('lodash');
...
const getFrameRange = async (file, globals) => {
  const lastScene = get(globals, 'scenes.last');
  if (lastScene) {
    return lastScene;
  } else {
...
const getSimpleTransformCmd = async (file, globals, transformCmd) => {
