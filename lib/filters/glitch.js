...
class FilterGlitch {
  ...
  async apply(file) {
    ...
    const result = exec(
      `ffmpeg -v warning -filter_complex "[0:v] fps=12,split [a][b];[a] palettegen=reserve_transparent=off:stats_mode=single [p];[b][p] paletteuse=new=1" -y -i "${file}" "${file}.gif" && rm "${file}" && mv "${file}.gif" "${file}"`,
      { silent: true }
    );
    if (result.code === 1) {
