...
class FilterCaptions {
  ...
  getFontSize(width, captions) {
    // Start reducing font-size after this many characters.
    // Totally just some pseudologic based on trial and error.
    const idealCount = 50;
    const captionArray = Array.isArray(captions) ? captions : [captions];
    const longestCaptionLength = captionArray.reduce(
      (memo, caption) => (caption.length > memo ? caption.length : memo),
      0
    );
    const captionFactor = Math.min(1, idealCount / longestCaptionLength);
