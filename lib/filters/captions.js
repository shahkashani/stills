...
class FilterCaptions {
  ...
  getCmdText(caption, width, color, offset = 0) {
    ...
    return `-pointsize ${fontSize} ${fontCmd} -size ${boxWidth}x -gravity South -fill ${color} -background none caption:"${cleanCaption}" -geometry +${offsetX}+${offsetY}`;
  }

  getStillCmd(file, width, caption) {
    const cmdBlackText = this.getCmdText(caption, width, 'black', 2);
    const cmdWhiteText = this.getCmdText(caption, width, 'white');
