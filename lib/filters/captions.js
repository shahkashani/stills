...
class FilterCaptions {
  ...
  getGifFrameIntervals(globals, numCaptions, numFrames) {
    const midpoint = get(globals, 'scenes.midpoint', Math.floor(numFrames / 2));
    ...
    if (numCaptions === 2) {
      return [[0, midpoint - 1], [midpoint, numFrames - 1]];
    }
  ...
  getGifCmd(file, globals, captions, width, numFrames) {
    const background = this.background;
  ...
  apply(file, imageInfo, globals) {
    ...
    const cmd =
      numFrames > 1
        ? this.getGifCmd(file, globals, captions, width, numFrames)
