...
class FilterEmbed {
  ...
  async applyFrame(frame, { numImage, numImages, numFrames }) {
    ...
    const orig = `orig-${file}`;
    copyFileSync(file, orig);
    ...
    this.positions.forEach(({ x, y, width, height, rotate, flip, flop }) => {
      ...
      const f2 = flop ? `-flop` : '';
      execCmd(
        `convert "${file}" -coalesce null: \\( "${orig}" -resize "${width}x${height}^" -gravity center -extent ${width}x${height} -gravity northwest ${r} ${f1} ${f2} \\) -geometry +${x}+${y} -layers composite "${file}"`
      );
    });
    unlinkSync(orig);
    execCmd(
