...
class FilterBlend {
  ...
  transform(overlayFrames, base, frame, count, total) {
    ...
    const overlayIndex = Math.ceil(overlayFrames.frames.length * (count / total));
  ...
  async apply(file, getInfo) {
    ...
    const availableDuration = getImageInfo(this.overlayFile).duration - duration;
