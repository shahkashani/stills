...
const processFile = (
  ...
  const promises = positions.map(position => {
    ...
    return new Promise((resolve, reject) => {
      const cmd = `ffmpeg -ss ${sec} -i "${file}" ${ffmpegCommand} "${outputPath}"`;
...
const makeGifs = (
  ...
  // convert "${outputPath}"  -coalesce -duplicate 1,-2-1 -quiet -layers OptimizePlus  -loop 0 "${outputPath}"
  return make(
    globPattern,
    inputFolder,
    outputFolder,
    '-v warning -t 0.5 -vf scale=640:-1:sws_dither=ed -y',
    'gif',
    num,
    min,
    max
