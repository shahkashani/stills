...
const drawEyes = (frame, face, { color, blur, debugColor, imperfection }) => {
  if (
  ...
  const fills = coordinates.map(({ x, y }) => {
    if (imperfection > 0) {
  ...
  execCmd(
    getRawDrawCommand(frame, fills, {
      fill: debugColor ? debugColor : color,
      effects: blur ? `-blur 0x${size * blur}` : ''
    })
  );
};
