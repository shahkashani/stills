const execCmd = require('../utils/exec-cmd');
const {
  getBoundingBox,
  isAngled,
  getFallbackCoordinates,
  getAngle,
  getDegreesBetween,
  getDistanceBetween,
  getCenter
} = require('../utils/faces');
const getRawDrawCommand = require('../utils/get-raw-draw-command');
const getImperfectCircle = require('../utils/get-imperfect-circle');

const MAX_DEGREES = 40;

const getFallback = (face, minSize, distance) => {
  const {
    height,
    center: { x, y }
  } = getFallbackCoordinates(face);

  const angle = getAngle(face);

  const baselineY = Math.round(y - height * 0.2);
  const useDistance = minSize * 3;
  const diffY = (height / 10) * (angle.roll / 2);

  return [
    {
      x: Math.round(x - useDistance),
      y: baselineY,
      width: minSize,
      height: minSize
    },
    {
      x: Math.round(x + useDistance),
      y: Math.round(baselineY + diffY),
      width: minSize,
      height: minSize
    }
  ];
};

const getFeaturesCmd = (
  frame,
  face,
  { color, blur, debugColor, imperfection, isFallback }
) => {
  const eye1 = face.landmarks.getLeftEye();
  const eye2 = face.landmarks.getRightEye();
  const bounds1 = getBoundingBox(eye1);
  const bounds2 = getBoundingBox(eye2);

  const { width: faceWidth } = face.detection.box;
  const minSize = Math.max(1, Math.floor(faceWidth / 20));
  const size = Math.max(minSize, Math.min(bounds1.width, bounds1.height));

  const degreesBetween = getDegreesBetween(eye1[0], eye2[0]);
  const useFallback =
    (Math.abs(degreesBetween) > MAX_DEGREES || isAngled(face)) && isFallback;
  const coordinates = useFallback
    ? getFallback(
        face,
        size,
        getDistanceBetween(getCenter(bounds1), getCenter(bounds2))
      )
    : [bounds1, bounds2];

  const commands = coordinates.map(({ x, y, width, height }) => {
    const centerX = x + width / 2;
    const centerY = y + height / 2;
    const startX = centerX + size / 2;
    const startY = centerY + size / 2;
    if (imperfection > 0) {
      return `-draw '${getImperfectCircle(
        centerX,
        centerY,
        size,
        size * (1 + imperfection)
      )}'`;
    } else {
      return `-draw 'circle ${startX},${startY} ${startX},${startY + size}'`;
    }
  });

  return getRawDrawCommand(frame, commands, {
    fill: useFallback && debugColor ? debugColor : color,
    effects: `-blur 0x${size * blur}`
  });
};

module.exports = async (
  frame,
  {
    color = '#eeeeee',
    debugColor = null,
    blur = 0.15,
    imperfection = 0,
    isFallback = false
  } = {}
) => {
  const faces = await frame.getFaces();
  if (faces.length === 0) {
    return;
  }
  for (const face of faces) {
    execCmd(
      getFeaturesCmd(frame, face, {
        color,
        imperfection,
        blur,
        debugColor,
        isFallback
      })
    );
  }
};
