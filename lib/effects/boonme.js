...
const getOutlineCmd = (
  ...
  const blurCmd = blur ? `-blur 0x${Math.ceil(width * blur)}` : '';
  const masks = draws.map(({ command, effects = '' }, i) => {
    const extendMask = effects
      ? `-gravity center -background black -extent ${maskSize} ${blurCmd} ${effects} -crop ${imageSize}+0+0`
      : blurCmd;
...
module.exports = async (
  frame,
  image,
  { blur = 0.3, modulate = '-1000%', useWave = false } = {}
) => {
