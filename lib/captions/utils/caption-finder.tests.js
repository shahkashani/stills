...
describe('CaptionFinder', () => {
  ...
  it('extends a single string search', async () => {
    ...
    expect(captions).toEqual([
      'We get a lot of people coming in here for the restaurant',
      "and don't come here all the time.",
  ...
  it('extends a single string search from a caption further into the search', async () => {
    ...
    expect(captions).toEqual([
      ...
      'AND MAYBE EVEN MORE OFF THE HOOK THAN THE FOOD',
      'IS THE GUY WHO STARTED IT ALL...',
      "THAT'S NOT A TUXEDO JACKET. THAT'S MY...",
      'MAD GREEK MICHAEL DABOUR.'
    ]);
  });
  ...
  it('extends a multi-string search', async () => {
    ...
    expect(name).toEqual('S27E05 Mex to the Max');
    expect(captions).toEqual([
      'You have a bakery. -Right, right.',
      'We get a lot of people coming in here for the restaurant',
      "and don't come here all the time.",
      'Homeschooled.',
      'MAN: You can come every meal'
    ]);
  });
