...
class GlitchPlugin {
  ...
  createGlitch(file) {
    ...
    const fix = exec(
      `ffmpeg -v warning -filter_complex "[0:v] fps=12,scale=w=540:h=-1,split [a][b];[a] palettegen=stats_mode=single [p];[b][p] paletteuse=new=1" -y -i "${file}" "${file}.gif" && rm "${file}" && mv "${file}.gif" "${file}"`,
      { silent: true }
    );
    if (fix.code === 1) {
