...
const { map, sample, first, random, findIndex, flatten } = require('lodash');
...
class GlobalsCaptions {
  ...
  async loadCaptions(word = null) {
    const { captions, captionFile, captionFileType } = word
      ? await this.loadTargetedCaptionFile(this.folder, word)
  ...
  getTargetedCaptions(num, word) {
    ...
    const index = findIndex(captions, (caption) => regexp.test(caption));
  ...
  async get(files, getImageInfo, globals) {
    ...
    let captions = word
      ? this.getTargetedCaptions(totalCaptions, word)
      : this.getSentenceCaptions(totalCaptions);
